...
function P = pdfclass
    ...
    function l=list(X)
      ...
      if isempty(X.clslist) %%%
        files = dir([fileparts(which('pdfclass')) '/*.csl']);
        for n={files.name};
          try
            cls = strrep(n{1}, '.csl', '');
            obj = eval(cls);
...
opt=struct('conf',[90],'view','pdf','axis',0,'col','bgrmyk','fontsize',[],'linewidth',[],'Xlim',[],'Ylim',[],'legend','auto','markersize',10);
    ...
    function pdfplot(varargin)
      ...
      %   $Revision: 1.8 $ $Date: 2010/03/23 11:48:34 $
      %$$
      ...
      opt=struct('view','pdf','axis',0,'s',[],'col','bgrmyk','fontsize',[],'linewidth',[],'Xlim',[],'Ylim',[],'legend','auto');
      opt=optset(opt,optvar);
      if opt.axis==0; opt.axis=gca; end
      for k=1:N;
        dist=M{k};
