...
if nargin<1
%    error('JLSS contructor expects an input argument: see help jlss')
else
   M=nargin;
   for i=1:M
       try
	   m{i}=varargin{1};
           ...
           disp(['LSS error code: ',ME.message])
           error(['JLSS constructor: Input argument ',num2str(i),' is not a valid LSS object'])
       end
   end
   for i=2:M
      if ~isequal(m{1}.nn,m{i}.nn)
	   error(['JLSS constructor: model ',num2str(i),' does not have the same nn as model 1'])
      end
   end
   Pi=ones(M)/M;
end
...
if isa(s1,'lti') & ~isa(s1,'ss')
   s1=lss(s1);
end
if isa(s2,'lti') & ~isa(s2,'ss')
   s2=lss(s2);
end
...
%
%   Algorithm:
%     1. ss.modred is called with automatic choice of model order,
...
A=s.A; B=s.B; C=s.C; D=s.D; Q1=s.Q; R1=s.R; S1=s.S; fs=s.fs; nn=s.nn;
if nn(2)==0
   error('ss.balreal requires a system with at least one input')
end
P=gram(s,'c');
Q=gram(s,'o');
[Up,Sp,Vp]=svd(P);
...
opt=optset(opt,varargin);
if isnan(m.fs)
    error('ss.kalman: only discrete time models')
end
[nx,nu,nv,ny]=size(m);
...
if nuz~=nu
   error('LSS.KALMAN: Number of inputss nu must be the same in model and signal')
end
...
if opt.alg==3
  if k>1, 
       error('ss.kalman: square root algorithm must have k=0 or k=1'), 
  end
  yhat=zeros(ny,N);
  Epsi=zeros(ny,N);
  Epsin=zeros(ny,N);
...
%
%   Examples:
%   ss2nl(rand(ss([2 2 2 2])))
%   See also: nl2lss, ss2armax, ss2arx, ltf.ltf2lss
...
%   In contrast to the function ss2ltf, this method works for MIMO systems
%
%   Any noise model is neglected in ss2ltf
%
%   Examples:
%   See also: ss2armax, ss2arx, ltf/ltf2lss
...
%
%   Examples:
%   See also: ss2ltf, ss2arx, ltf/ltf2lss
...
%    Example: sys=zeros(ss([1,1,0,2,2]));
...
%
%   Example:
%      rand(ss([3,2,0,1]) generates a random model with 3 states,
...
%
%   See also: ltf.fix, ss.uncertain, ss.estimate
...
%     y=step(G);
%     plot(y)
%   See also: ss.impulse, ss.simulate
...
function display(m)
%DISPLAY gives a ascii formatted printout of the LSS object
%   display(sys)  or  sys
format='%11.2g';
if isnan(m.fs)
