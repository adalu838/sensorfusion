...
elseif ~isequalwithequalnans(z1.fs,z2.fs)
   ...
   z.x=[z1.x z2.x];
   z.y=[z1.y z2.y];
   z.yMC=cat(3,z1.yMC,z2.yMC);
   z.xMC=cat(3,z1.xMC,z2.xMC);
end
end
...
elseif ~isequalwithequalnans(z1.fs,z2.fs)
   ...
   z.x=[z1.x; z2.x];
   z.u=[z1.u; z2.u];
   z.yMC=cat(2,z1.yMC,z2.yMC);
   z.xMC=cat(2,z1.xMC,z2.xMC);
...
function display(s)
%DISPLAY function for SIG objects
if isempty(s.y)
...
function stem(varargin)
%STEM calls sig.sigplot with view set to stem
%   See sig.sigplot for help
sigplot(varargin{:},'view','stem')
end

function staircase(varargin)
%STAIRCASE calls sig.sigplot with view set to staircase
%   See sig.sigplot for help
sigplot(varargin{:},'view','staircase')
end

function uplot(varargin)
...
for k=1:length(varargin)
	...
	plot(y.x(:,i),y.x(:,j),[col(kk),'.-'],'parent',opt.axis,'linewidth',opt.linewidth)
...
elseif isa(z2,'sig') % op on two signals
    if size(z1.y)==size(z2.y) & size(z1.fs)==size(z2.fs) & size(z1.t(:))==size(z2.t(:))
        z=z1;
...
if ~isempty(z.yMC)
    for k=1:z.MC
        [zdtmp,zttmp]=detrend( sig(shiftdim(z.yMC(k,:,:),1),1),order ,varargin{:});
        ydMC(k,:,:)=zdtmp.y;
        ytMC(k,:,:)=zttmp.y;
    end
else
  ...
  for j=1:ny(k)
    ...
    for i=1:nuk
      ...
      if ~isempty(z.yMC)
        ...
        if opt.conf>0 & opt.conf<=100
            p=confband(z.t,z.yMC(:,:,j)','plot',opt.col(kk),opt.axis,opt.conf,opt.conftype);
            %if legon==0; legend([p1(2) p2 p4],'Nominal','Confidence bound','Median','parent',opt.axis); legon=1; end
        end
      end
      if ~isempty(z.Py)
        if opt.conf>0 & opt.conf<=100
            p=confband2(z.t,z.y(:,j),z.Py(:,j,j),'plot',opt.col(kk),opt.axis,opt.conf,opt.conftype);
        end
      end
      if nu(k)==0
        try
           title(opt.axis,[z.ylabel{j}])
        end
